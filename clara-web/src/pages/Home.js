  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'DM Sans, sans-serif', padding: '60px 18px 90px' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ height: 32, width: 140, borderRadius: 12, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 20 }} />
        <div style={{ height: 180, borderRadius: 24, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 18 }} />
        <div style={{ height: 80, borderRadius: 14, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 14 }} />
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[1,2,3].map(i => <div key={i} style={{ height: 56, borderRadius: 14, background: 'white', animation: 'pulse 1.4s ease infinite', animationDelay: `${i*0.1}s` }} />)}
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'DM Sans, sans-serif', position: 'relative' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (min-width: 768px) {
          ...
          .home-grid { display: grid !important; grid-template-columns: 1fr 1fr; gap: 14px; }
      ...
      <div className="desktop-nav" style={{ padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[['Inicio', '/home', true], ['Gastos', '/movimientos', false], ['Plan', '/plan', false], ['Hogar', '/hogar', false]].map(([label, to, active]) => (
            <button key={label} onClick={() => navigate(to)} style={{ padding: '7px 13px', borderRadius: 10, fontSize: 12, color: active ? C.green : C.text2, cursor: 'pointer', border: active ? `1px solid rgba(94,240,176,0.18)` : '1px solid transparent', background: active ? 'rgba(94,240,176,0.07)' : 'transparent', fontFamily: 'DM Sans, sans-serif' }}>{label}</button>
          ))}
        </div>
      ...
      {/* Content */}
      <div className="page-content" style={{ position: 'relative', zIndex: 1, padding: '0 0 90px' }}>

        {/* Mobile header */}
        <div className="mobile-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 22px 10px' }}>
              <div>
            ...
            <div className="home-grid" style={{ padding: '0 16px' }}>
            <div>
      ...
      <div className="bottom-nav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 80, background: 'rgba(8,13,26,0.88)', backdropFilter: 'blur(30px)', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-around', paddingTop: 10, zIndex: 10 }}>
        {[
          { icon: <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>, label: 'Inicio', to: '/home', active: true },
          ...
          <div key={label} onClick={() => navigate(to)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', padding: '4px 12px', color: active ? C.green : C.text3 }}>
