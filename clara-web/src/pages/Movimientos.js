  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'DM Sans, sans-serif', padding: '60px 18px 90px' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ height: 32, width: 170, borderRadius: 12, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 20 }} />
        <div style={{ height: 140, borderRadius: 24, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 18 }} />
        <div style={{ height: 44, borderRadius: 12, background: 'white', animation: 'pulse 1.4s ease infinite', marginBottom: 14 }} />
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[1,2,3,4,5].map(i => <div key={i} style={{ height: 60, borderRadius: 14, background: 'white', animation: 'pulse 1.4s ease infinite', animationDelay: `${i*0.08}s` }} />)}
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div className="bottom-nav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 80, background: 'rgba(8,13,26,0.88)', backdropFilter: 'blur(30px)', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-around', paddingTop: 10, zIndex: 10 }}>
        {[
          ...
          { icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="20" height="20"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>, label: 'Gastos', to: '/movimientos', active: true },
        ].map(({ icon, label, to, active }) => (
          <div key={label} onClick={() => navigate(to)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', padding: '4px 12px', color: active ? C.blue : C.text3 }}>
